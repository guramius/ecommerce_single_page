...
// import DisplayAddCount from "../../atoms/displayAddCount/DisplayAddCount"
import minuse from '../../images/minuse.png'
...
const Counter = () => {
    ...
    const addCountBUtton = () => {
        setAddCount(addCount + count)
    } 
  return (
    <div className="flex items-center">
        <div className="flex items-center rounded-[10px] bg-[#F6F8FD] px-[23px] py-[16px] gap-[45px] mr-[16px]">
            <img className="cursor-pointer" onClick={decreaseCount} src={minuse} alt="minuse" />
            <p>{count}</p>
            <img className="cursor-pointer" onClick={incriseCount} src={plus} alt="pluse" />
        </div>
        <div onClick={addCountBUtton} className="flex items-center justify-center gap-[15px]  w-[272px] h-[56px] flex-shrink-0 rounded-[10px] bg-[#FF7E1B] shadow-[0px_20px_50px_-20px_#FF7E1B] cursor-pointer">
            ...
            <p className="text-[20px]">{addCount}</p>
            {/* <DisplayAddCount amount={addCount}/> */}
        </div>
    </div>
  )
}
